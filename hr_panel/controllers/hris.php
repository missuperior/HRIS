...
class Hris extends CI_Controller {
                ...
                'hris_username'   => $result->hris_username,
                 ...
                 'company_name'    => $result->company_name,
                  'company_type'    => $result->company_type,
                    'company_image'   => $result->image_path,
                     'css_type'   => $result->css_type,
                );
  ...
  public function add_employee_form() {
    ...
    $this->login_check();
    $result['shifts']= $this->Hris_model->getAllshift();
   // print_r($result); die;
...
//     
      ...
      $grade              = $this->input->post('record_grades');
...
//                print_r($record_data);
      ...
      for($i=0; $i<count($record_department); $i++){
          ...
          'grade_level_id'              =>  $grade [$i],
          'shift_id'                    => $shift_time[$i],
...
//                print_r($license_data);
      ...
      if($employee_id)
      {
           $employee_record = $this->Hris_model->employee_info_maker($employee_id);
   ...
   foreach($employee_record as $row)
         ...
         $this->Hris_model->add_employee_detail($emp_data);
            }   
  ...
  public function edit_employee() {
    ...
    $result['document']    = $this->Hris_model->getDocument($emp_id);
    $result['docs'] = $this->Hris_model->get_all_documents();
    $result['department']  = $this->Hris_model->get_departs($com_id);
    ...
    $result['campus']      = $this->Hris_model->getAllCampuses();
    $result['company']     = $this->Hris_model->getAllCompany();
    $result['grade']       = $this->Hris_model->Departmentgrade($dept_id);
...
//      print_r($document_data);
      ...
      $employee_record = $this->Hris_model->employee_info_maker($emp_id);
   ...
   foreach($employee_record as $row)
         ...
         $this->Hris_model->update_employee_detail($emp_data,$emp_id);
            }
   ...
   // get grade by department id
   function grade_by_department_id()
    {
        
    $this->login_check();    
    $department_id = $this->input->get('dept_id');
    
    $result['grade'] = $this->Hris_model->Departmentgrade($department_id);  
    
    
        
    if(!empty($result))
    {
      $this->load->view('hris/employee/department_grade', $result);
    }
    else
    {
      echo "No Designation";
...
//    }
    ...
    public function get_ajax_salary() {
        $company_id = $_GET['company_id'];
        $result['page_name'] = $_GET['page_name'];


        $result['department'] = $this->Hris_model->get_departs($company_id);
        if ($result['page_name'] == 'report') {
            $this->load->view('attendance/reports/salary_partial', $result);
        } else {
            $this->load->view('hris/employee/salary_partial', $result);
        }
    }

    public function get_company_dpt() {
        $company_id = $_GET['company_id'];
        $result['department'] = $this->Hris_model->get_departs($company_id);
        $this->load->view('hris/employee/department_partial', $result);
    }
...
';
            ...
            $config = Array(
                ...
                'smtp_host' => 'ssl://smtp.googlemail.com',
                'smtp_port' => 465,
                'smtp_user' => 'eslip.superior@gmail.com',
                'smtp_pass' => 'Slipsuperior123', // change it to yours
                'mailtype' => 'html',
                'charset' => 'iso-8859-1',
                'wordwrap' => TRUE,
            );
    ...
    public function benefit_form() {
        ...
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/benefit/benefit_form');
        $this->load->view('hris_ace/hris_footer');
    }
    ...
    public function add_benefit() {
        ...
        $data['created_by'] = $this->session->userdata('hris_id');
        ...
        $id = $this->Hris_model->addBenefit($data);
        redirect('hris/all_benefits');
        //echo $id; die;
    }

    public function all_benefits() {
        // $company = $this->session->userdata('company_id');
        $result['benefits'] = $this->Hris_model->allBenefit('');

        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/benefit/benefit_all', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    public function delete_benefit() {
        $id = $this->uri->segment(3);
        $delete = $this->Hris_model->delete_benefit($id);
        if ($delete) {
            $this->session->set_userdata('success', 'Seccessfully Deleted');
        } else {
            $this->session->set_userdata('error', 'Error Whhile deleting item');
        }
        redirect('hris/view_salary_info');
    }

    public function edit_benefit_form() {
        $id = $this->uri->segment(3);
        $result['benefits'] = $this->Hris_model->allBenefit($id);
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/benefit/edit_benefit_form', $result);
        $this->load->view('hris_ace/hris_footer');
    }
    ...
    public function edit_benefit() {
        ...
        $data['benefit_id'] = $data['title'] = $_POST['title'];
        $data['type'] = $_POST['type'];
        $data['default_value'] = $_POST['default_value'];
        $data['status'] = $_POST['status'];
        $res = $this->Hris_model->editBenefit($_POST['id'], $data);
        if ($res) {
            $this->session->set_userdata('success', 'Seccessfully Updated');
        } else {
            $this->session->set_userdata('error', 'Error While updated');
        }
        redirect('hris/all_benefits');
    }
    ...
    public function level_form() {
        ...
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/level/level_form');
        $this->load->view('hris_ace/hris_footer');
    }
    ...
    public function add_level() {
        ...
        $id = $this->Hris_model->addLevel($data);
        redirect('hris/all_levels');
        //echo $id; die;
    }

    public function all_levels() {
        // $company = $this->session->userdata('company_id');
        $result['benefits'] = $this->Hris_model->allLevel('');


        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/level/level_all', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    public function delete_level() {
        $id = $this->uri->segment(3);
        $delete = $this->Hris_model->delete_level($id);
        if ($delete) {
            $this->session->set_userdata('success', 'Seccessfully Deleted');
        } else {
            $this->session->set_userdata('error', 'Error Whhile deleting item');
    ...
    public function edit_level_form() {
        $id = $this->uri->segment(3);
        $result['level'] = $this->Hris_model->allLevel($id);
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/level/edit_level_form', $result);
        $this->load->view('hris_ace/hris_footer');
    }
...
//        $data['created_by'] = $this->session->userdata('hris_id');
        ...
        $data['title'] = $_POST['title'];
        $data['status'] = $_POST['status'];
        $res = $this->Hris_model->editLevel($_POST['id'], $data);
        if ($res) {
            $this->session->set_userdata('success', 'Seccessfully Updated');
        } else {
            $this->session->set_userdata('error', 'Error While updated');
    ...
    public function grade_form() {
        ...
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/grade/grade_form');
        $this->load->view('hris_ace/hris_footer');
    }
    ...
    public function add_grade() {
        ...
        $data['company_id'] = $this->session->userdata('company_id');
        $data['created_date'] = date("Y-m-d H:i:s");
        $data['class'] = $_POST['title'];
        $data['status'] = $_POST['status'];

        $id = $this->Hris_model->addGrade($data);
        redirect('hris/all_grades');
        //echo $id; die;
    }

    public function all_grades() {
        // $company = $this->session->userdata('company_id');
        $result['grades'] = $this->Hris_model->allGrade('');

        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/grade/grade_all', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    public function delete_grade() {
        $id = $this->uri->segment(3);
        $delete = $this->Hris_model->delete_grade($id);
        if ($delete) {
            $this->session->set_userdata('success', 'Seccessfully Deleted');
        } else {
            $this->session->set_userdata('error', 'Error Whhile deleting item');
    ...
    public function edit_grade_form() {
        $id = $this->uri->segment(3);
        $result['grade'] = $this->Hris_model->allGrade($id);
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/grade/edit_grade_form', $result);
        $this->load->view('hris_ace/hris_footer');
    }
...
//        $data['created_by'] = $this->session->userdata('hris_id');
        ...
        $data['class'] = $_POST['title'];
        $data['status'] = $_POST['status'];
        $res = $this->Hris_model->editgrade($_POST['id'], $data);
        if ($res) {
            $this->session->set_userdata('success', 'Seccessfully Updated');
        } else {
            $this->session->set_userdata('error', 'Error While updated');
    ...
    public function add_grade_benefit_form() {
        $company = $this->session->userdata('company_id');
        $result['benefits'] = $this->Hris_model->allBenefit();
        $result['grade'] = $this->Hris_model->getGradesCampousWise($company);
        $result['levels'] = $this->Hris_model->getLevelCampusWise($company);
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/grade/benefit_grade', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    public function add_grade_benefit() {
        $data['company_id'] = $this->session->userdata('company_id');
        //$data['benefit_id'] = $_POST['benefit'];
        $data['grade_id'] = $_POST['grade'];
        $data['level_id'] = $_POST['level'];
        $res = $this->Hris_model->addGradeLevel($data);
        ...
        redirect('hris/add_grade_benefit_form');
    }
    ...
    public function salary_form() {
        ...
        $result['benefits'] = $this->Hris_model->allBenefit('');
        $result['grades'] = $this->Hris_model->getGradesCampousWise($company);
        $result['levels'] = $this->Hris_model->getLevelCampusWise($company);
        ...
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/salary/salary_form', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    public function add_salary() {
        //$this->login_check();   
        //  echo $_POST['2_ben_dv']; die;
        // echo $_POST[$_POST['benefits'][0].'_ben_dv']; die;
//        echo '<pre>';
//        var_dump($_POST); die;
//        
        ...
        $session_msg = 0;
        $created_by = $this->session->userdata('hris_id');
        $company = $this->session->userdata('company_id');

        $data_salery['min'] = $_POST['min_sal'];
        $data_salery['mid'] = $_POST['mid_sal'];
        $data_salery['max'] = $_POST['max_sal'];
        $data_salery['sub_scale'] = $_POST['sub_scale'];
        ...
        foreach ($_POST['level'] as $lev) {
            ...
            $is_exist = $this->Hris_model->checkGradeLevel($grade_id, $lev, $company, $department);
            if (empty($is_exist)) {
                $res = $this->Hris_model->addGradeLevel($data);

                if ($res) {
                    $data_salery['grade_level_id'] = $res;
                    $data_salery['created_date'] = date("Y-m-d H:i:s");
                    $data_salery['created_by'] = $created_by;
                    $gradeLevelSalaryRes = $this->Hris_model->addGradeLevelSalary($data_salery);
                }

                foreach ($_POST['benefits'] as $benefit) {

                    $data_benefit['grade_level_id'] = $res;
                    $data_benefit['benefit_id'] = $benefit;
                    $data_benefit['value'] = $_POST[$benefit . '_benefit_value'];
                    $data_benefit['created_date'] = date("Y-m-d H:i:s");
                    $data_benefit['created_by'] = $created_by;
                    $benefites = $this->Hris_model->addGradeLevelBenefit($data_benefit);
                }
            } else {
                $session_msg++;
            }
        ...
        if ($session_msg != 0) {
            $this->session->set_userdata('error', $session_msg . ' levels are already added to system');
        }
    ...
    public function all_salaries() {
        // $company = $this->session->userdata('company_id');
        // $result['benefits'] = $this->Hris_model->allSalary();


        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/salary/salary_all');
        $this->load->view('hris_ace/hris_footer');
    }

    public function delete_salary() {
        $id = $this->uri->segment(3);
        $delete = $this->Hris_model->delete_salary($id);
        if ($delete) {
            $this->session->set_userdata('success', 'Seccessfully Deleted');
        } else {
            $this->session->set_userdata('error', 'Error While deleting item');
        }
        redirect('hris/view_salary_info');
    }

    public function edit_salary_form() {
        $id = $this->uri->segment(3);
        $result['level'] = $this->Hris_model->allSalary($id);
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/level/edit_level_form', $result);
        $this->load->view('hris_ace/hris_footer');
...
//        $data['created_by'] = $this->session->userdata('hris_id');
        ...
        $data['title'] = $_POST['title'];
        $data['status'] = $_POST['status'];
        $res = $this->Hris_model->editSalary($_POST['id'], $data);
        if ($res) {
            $this->session->set_userdata('success', 'Seccessfully Updated');
        } else {
            $this->session->set_userdata('error', 'Error While updated');
        }
        redirect('hris/all_salaries');
    }

    public function add_grade_benefit_2() {
        
    }

    public function view_salary_info() {
        $result['company'] = $this->Hris_model->getCampany();

        //  $result['salary'] = $this->Hris_model->viewSalary($company, $department);

        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/salary/salary_all', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    function get_ajax_designation() {

        $department = $_GET['department'];
        $result['designation'] = $this->Hris_model->getDesignationDepartmentWIse($department);
        $this->load->view('hris/employee/designation_partial', $result);
    }

    function get_ajax_grade_level() {
        $department = $_POST['department'];
        $compnay = $_POST['company'];

        $page_from = $_POST['page_for'];
        ...
        if ($page_from == 'report') {
            $result['salary'] = $this->Hris_model->grade_benefit($compnay, $department);
            $result['info'] = $this->Hris_model->getCompanyDeapermentName($compnay,$department);
            
            
          
            $this->load->view('hris_ace/hris_header');
            $this->load->view('hris/report/grade_benefit_report', $result);
            $this->load->view('hris_ace/hris_footer');
        } else {
             $result['salary'] = $this->Hris_model->viewSalary($compnay, $department);
            $this->load->view('hris/employee/grade_level_partial', $result);
        }
    }

    function emp_department_report_form() {
        $result['company'] = $this->Hris_model->getCampany();

        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/report/emp_dep_form', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    function get_emp_department_report() {
        $dept_id = $this->input->post('department');
        $comp_id = $this->input->post('company');

        $result['employee'] = $this->Attendance_model->searchEmpsal($dept_id, $comp_id);
        $result['info'] = $this->Hris_model->getCompanyDeapermentName($comp_id, $dept_id);

        $this->load->view('hris_ace/hris_header');
        //$this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/report/department_employee_report', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    function company_overall_report_form() {
        $result['company'] = $this->Hris_model->getCampany();

        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/report/com_overall_form', $result);
        $this->load->view('hris_ace/hris_footer');
    }

    function get_emp_com_overall_report() {
        $comp_id = $this->input->post('company');

        $result['employee'] = $this->Hris_model->companyOverAllReport($comp_id);
        $result['info'] = $this->Hris_model->getCompanyDeapermentName($comp_id, '');
        $result['departments'] = $this->Hris_model->totalDepartmentCompanyWise($comp_id);
        
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris/report/company_overall_report', $result);
        $this->load->view('hris_ace/hris_footer');
    }
    
    
    function grade_company_form(){
        $result['company'] = $this->Hris_model->getCampany();
        $result['report_for'] = 'grade';

        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris_ace/hris_side_menu');
        $this->load->view('hris/report/grade_form', $result);
        $this->load->view('hris_ace/hris_footer'); 
    }
    
    function grade_company_report(){
         $comp_id = $this->input->post('company');

        $result['grades'] = $this->Hris_model->companyGradeReport($comp_id);
        $result['info'] = $this->Hris_model->getCompanyDeapermentName($comp_id, '');
        $result['from'] = 'grade';
       
        $this->load->view('hris_ace/hris_header');
        $this->load->view('hris/report/company_grade_report', $result);
        $this->load->view('hris_ace/hris_footer');
    }
    
    function level_company_form(){
        $result['company'] = $this->Hris_model->getCampany();
        $result['report_for'] = 'level';
    ...
    function level_company_report(){
        $comp_id = $this->input->post('company');

        $result['grades'] = $this->Hris_model->companyLevelReport($comp_id);
        $result['info'] = $this->Hris_model->getCompanyDeapermentName($comp_id, '');
        $result['from'] = 'level';
      ...
      // Add company leave 
  public function add_leavetype_form() {

    $this->login_check();
    
    $this->load->view('hris_ace/hris_header');
    $this->load->view('hris_ace/hris_side_menu');
    $this->load->view('hris/leavetype/add_leavetype');
    $this->load->view('hris_ace/hris_footer'); 
  }

  // Add company in database
  public function add_leave_company() {

    $this->login_check();

    $this->form_validation->set_rules('leave_type_name', 'Leave Type Name', 'required');

    if ($this->form_validation->run() == FALSE) {
      
      $this->load->view('hris_ace/hris_header');
      $this->load->view('hris_ace/hris_side_menu');
      $this->load->view('hris/leavetype/add_leavetype', $result);
      $this->load->view('hris_ace/hris_footer'); 
    } 
    else {
      $leave_data = array(
          'leavetype' => $this->input->post('leave_type_name')
      );
      
      // check Company already exitsts
      $res = $this->Hris_model->checkleave($leave_data);
      if ($res) {
        $this->session->set_userdata('error_msg', 'Leave Type Name Already Exists');
        redirect('hris/add_leavetype_form');
      } 
      else {
        $result = $this->Hris_model->addleavetype($leave_data);
        
        if ($result) {
          $this->session->set_userdata('success_msg', 'Leave Type Added Successfully');
          redirect('hris/view_leavetype');
        }
      }
    }
  }

  // display all the leave type 
  public function view_leavetype() {
    $this->login_check();

    $result['leavetype'] = $this->Hris_model->getAllLeaveType();

    $this->load->view('hris_ace/hris_header');
    $this->load->view('hris_ace/hris_side_menu');
    $this->load->view('hris/leavetype/view_leavetype', $result);
    $this->load->view('hris_ace/hris_footer');
  }
  ...
  public function edit_leavetype() {
    ...
    $result['leave'] = $this->Hris_model->getleavetype($lev_id);
   
    $this->load->view('hris_ace/hris_header');
    $this->load->view('hris_ace/hris_side_menu');
    $this->load->view('hris/leavetype/edit_leavetype', $result);
    $this->load->view('hris_ace/hris_footer');
  }
  ...
  public function update_leavetype() {
    ...
    $this->login_check();    
    $lev_id = $this->input->post('leave_type_id');
    ...
    else
    {
      ...
      $res = $this->Hris_model->checkleave($leave_data);
      if ($res) {
        $this->session->set_userdata('error_msg', 'Leave Type Already Exists');
    ...
    $this->load->view('hris_ace/hris_footer');
        ...
        $result = $this->Hris_model->updateleavetype($lev_id, $leave_data);

        if ($result) {
          $this->session->set_userdata('success_msg', 'leave type updated Successfully');
          redirect('hris/view_leavetype');
        }
      }
  ...
  //---------------Start of leave count add----------------\\
  function addleavepolicyform()
  {
       $this->login_check();
       $result['company'] = $this->Hris_model->getCampany();
       $result['leavetype'] = $this->Hris_model->getAllLeaveType();
    ...
    $this->load->view('hris_ace/hris_header');
    $this->load->view('hris_ace/hris_side_menu');
    $this->load->view('hris/leavepolicy/add_leavepolicy', $result);
    $this->load->view('hris_ace/hris_footer');
  }
  function add_leave_policy()
  {
       $this->login_check();
         if($_POST['company']=='' && $_POST['leavetype']=='' && $_POST['leavecount']=='')
             {
             
            $result['company'] = $this->Hris_model->getCampany();
            $result['leavetype'] = $this->Hris_model->getAllLeaveType();
          //  print_r($result); die;
            $this->session->set_userdata('error_msg', 'Fill All Fields ');
            $this->load->view('hris_ace/hris_header');
            $this->load->view('hris_ace/hris_side_menu');
            $this->load->view('hris/leavepolicy/add_leavepolicy', $result);
            $this->load->view('hris_ace/hris_footer');
      ...
      $leave_data = array(
          'compancy_id' => $this->input->post('company'),
          'leaveid' => $this->input->post('leavetype'),
          'leavecount' => $this->input->post('leavecount'),
          
      );
     
      // check Company already exitsts
      $result = $this->Hris_model->addleavepolicy($leave_data);
      if($result)
          {
           $this->session->set_userdata('success_msg', 'leave policy Successfully');
          redirect('hris/view_leavepolicy');
          }
          else
              {
              $this->session->set_userdata('success_msg', 'leave policy Not added');
          redirect('hris/view_leavepolicy');
              }
             }
  }
  function view_leavepolicy()
  {
       $this->login_check();
    $result['leavepolice'] = $this->Hris_model->getleavepolicy();
    $this->load->view('hris_ace/hris_header');
            $this->load->view('hris_ace/hris_side_menu');
            $this->load->view('hris/leavepolicy/view_leavepolicy', $result);
            $this->load->view('hris_ace/hris_footer');
    
  }
  function edit_leavepolicy()
  {
       $this->login_check();
       $lev_policy = $this->uri->segment(3);
        $result['company'] = $this->Hris_model->getCampany();
        $result['leavetype'] = $this->Hris_model->getAllLeaveType();
        $result['leavepolicy'] = $this->Hris_model->getleavepolicybyid($lev_policy);
   
    $this->load->view('hris_ace/hris_header');
    $this->load->view('hris_ace/hris_side_menu');
    $this->load->view('hris/leavepolicy/edit_leavepolicy', $result);
    $this->load->view('hris_ace/hris_footer');
  }
  function update_leavepolicy()
  {
       $this->login_check();
      $id = $this->input->post('leavepolicyid');
       if($_POST['company']=='' && $_POST['leavetype']=='' && $_POST['leavecount']=='')
             {
             
            $result['company'] = $this->Hris_model->getCampany();
            $result['leavetype'] = $this->Hris_model->getAllLeaveType();
            $result['leavepolicy'] = $this->Hris_model->getleavepolicybyid($id);
            $this->session->set_userdata('error_msg', 'Fill All Fields ');
            $this->load->view('hris_ace/hris_header');
            $this->load->view('hris_ace/hris_side_menu');
            $this->load->view('hris/leavepolicy/add_leavepolicy', $result);
        ...
        $result = $this->Hris_model->updateleavepolicy($id,$leave_data);
      if($result)
          {
           $this->session->set_userdata('success_msg', 'Update leave policy Successfully');
          redirect('hris/view_leavepolicy');
          }
          else
              {
              $this->session->set_userdata('success_msg', 'Not Update leave policy Successfully');
          redirect('hris/view_leavepolicy');
              }
             }
  }
  //---------------End of leave count end----------------\\
