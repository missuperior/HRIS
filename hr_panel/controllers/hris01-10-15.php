...
echo " I am ".$this->get_age("1985-01-21") ." years old";
 ...
 function getRandomCode() {
        $an = "1234567890abcdefghijklmnopqrstuvwxyz";
        $su = strlen($an) - 1;
    ...
    public function generate_employee_logins() {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if (isset($_POST['gen_password']) && (!empty($emp_id))) {
                $config = Array(
                  'protocol'  => 'smtp',
                  'smtp_host' => 'mail.google.com',
                  'smtp_port' => 25,
                  'smtp_user' => 'adeel.michael@superior.edu.pk', // change it to yours
                  'smtp_pass' => 'happy%today', // change it to yours
                  'mailtype'  => 'html',
                  'charset'   => 'iso-8859-1',
                  'wordwrap'  => TRUE
              );
              ...
              $this->load->library('email', $config);
              $this->email->set_newline("\r\n");
              $this->email->from('hr@superior.edu.pk'); // change it to yours
              $this->email->to($email[$i]);// change it to yours
              $this->email->subject('Welcome!');
              $this->email->message(
                   $message   
                      );
              $this->email->send();
              $this->Hris_model->add_gen_passwords($emp_id[$i], $email[$i], $status, $logins);
            }
        }
        redirect('hris/view_logins');
    }
