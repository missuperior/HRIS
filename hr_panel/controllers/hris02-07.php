...
//                print_r($license_data);
            ...
            $res = $this->Hris_model->updateEmployee($emp_id, $record_data, $ad_data, $sal_array, $bank_data, $fac_array, $contact_data, $dependent_array, $social_media_data, $emer_array, $med_array, $exp_array, $ref_array, $doc_array, $edu_array, $training_array, $skill_array, $lang_array, $mem_array, $license_array,
                                                        $document_id,
                                                        $emp_record_id,
                                                        $emp_salary_id,
                                                        $bank_id,
...
//    $this->form_validation->set_rules('d_cnic','Dependent CNIC', 'required');
    ...
    $this->form_validation->set_rules('work_email', 'work_email', 'required');
    $this->form_validation->set_rules('permanent_address', 'Permanant Address', 'required');
    $this->form_validation->set_rules('temporary_address', 'Temporary Address', 'required');
    $this->form_validation->set_rules('correspondence_address', 'Correspondence Address', 'required');
    
    // for Contact Tab (Personal Contact Details) Validation
    
    $this->form_validation->set_rules('e_name','Emergency Contact Person', 'required');
    $this->form_validation->set_rules('e_relationship','Emergency Relationship', 'required');
    $this->form_validation->set_rules('e_phone','Emergency  Phone', 'required');
    //$this->form_validation->set_rules('e_email','Emergency Email', 'required');
    $this->form_validation->set_rules('e_address','Emergency Address', 'required');  
    
    
    //   ******************  For Qualification Tab Validation 
    
    $this->form_validation->set_rules('education_title', 'Degree Title', 'required');     
    $this->form_validation->set_rules('education_grade', 'Grade/CGPA', 'required'); 
    $this->form_validation->set_rules('education_institute', 'Institute', 'required'); 
    $this->form_validation->set_rules('education_country', 'Country', 'required'); 
    $this->form_validation->set_rules('education_year', 'Completion Year', 'required'); 
...
//                print_r($facility_data);
      ...
      if ($_FILES['emp_pic']['name']) 
      {
                $empImageName = time().$_FILES['emp_pic']['name'];
                $config['upload_path'] = 'assets/EmployeeImages';
                $config['file_name'] = $empImageName;
                $config['overwrite'] = true;
                $config["allowed_types"] = 'jpg|jpeg|png|gif';
                ...
                if (in_array($ext1, $allowedExts1)) {
                    if ($_FILES["resume"]["error"] > 0) {                       
                        ...
                        @move_uploaded_file($_FILES["document_attach"]["tmp_name"][$i] , $images_path.'/'.$document_name );                        
                            ...
                            echo 'Error 2<br>'.$error1 = $this->session->set_userdata('error',  "Invalid Resume" );                            
                }
